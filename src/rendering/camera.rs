...
impl CamInfo {
    pub fn update_self(&mut self, camera: &Camera, size: &winit::dpi::PhysicalSize<u32>){
...
impl CamManager {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue, camera: Camera, size: &winit::dpi::PhysicalSize<u32>) -> Self {
        let mut cam_info = CamInfo::default();
        cam_info.update_self(&camera, size);
    ...
    pub fn update_buffers(&mut self, queue: &wgpu::Queue){
        queue.write_buffer(&self.camera_buffer, 0, bytemuck::cast_slice(&[self.cam_info]));
    }
}
